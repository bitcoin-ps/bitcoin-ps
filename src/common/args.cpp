...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\BitcoinPs
    //   new: C:\Users\Username\AppData\Local\BitcoinPs
    // macOS: ~/Library/Application Support/BitcoinPS
    // Unix-like: ~/.bitcoin-ps
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinPS";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "BitcoinPS";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/BitcoinPS";
#else
    // Unix-like
    return pathRet / ".bitcoin-ps";
#endif
#endif
}
